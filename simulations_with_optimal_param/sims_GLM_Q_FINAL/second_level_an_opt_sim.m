...
%         buff_LA{state}(:,4)=dat.LIP(state,dat.k(state,:)>0 & dat.optim(state,:)==1)';
        buff_LA{state}(:,4)=dat.LIP(dat.k(state,:)>0)';
        buff{state}(:,5)=dat.respside(state,dat.k(state,:)>0)';
        
        buffV=dat.V(state,dat.k(state,:)>0,:);
        buff{state}(:,6)=squeeze(buffV(:,buff{state}(:,5)))';
        buff{state}(:,7)=dat.b(dat.k(state,:)>0)';
        buff{state}(:,8)=dat.D(state,dat.k(state,:)>0)';
    ...
    D_inf=squeeze(mean(group(:,5:6,8),1));
    D_uninf=squeeze(mean(group(:,7:8,8),1));
    D2_inf=squeeze(mean(group(:,5:6,9),1));
    D2_uninf=squeeze(mean(group(:,7:8,9),1));
...
perf_res=[opt_inf opt_uninf compl_inf compl_uninf boost_inf boost_uninf LA_inf LA_uninf V D_inf D_uninf D2_inf D2_uninf];
