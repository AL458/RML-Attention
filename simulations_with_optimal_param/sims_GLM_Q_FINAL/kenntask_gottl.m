...
%             end
           ...
           %trial starts randomly from state 1 or 2 whether it's HR (s1)
            ...
            while s<=nstate %within trial state transitions
                ...
                b=1;%action(AccBoost,s,[1:arg.nactions_boost],1);%boost selection 
...
% %             linear attractor network to detect motion direction
                ...
                %gain Q modulation
                LIP=arg.constAct.NElesion*max(AccAct.V(s,1:2))*LIP;
...
%                 dat.LIP(s,trial)=max(max(LIP));
%                 dat.b(s,trial)=b;
                dat.D(s,trial)=abs(AccAct.D(s,res));
                dat.D2(s,trial)=abs(AccBoost.D(s,res));
